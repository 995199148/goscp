...
func main() {
	password := flag.String("p", "123456", "密码")
	srcPath := flag.String("src", "root@127.0.0.1:/root/test.txt", "源地址")
	dstPath := flag.String("dst", "/root/test.txt", "目的地址")
	port := flag.Int64("P", 22, "Port")
	flag.Parse()
	host := ""
	user := ""
	opstype := 1
	if strings.Contains(*srcPath, "@") {
		...
		user = srcPathlist[0]
		hostpath := strings.Split(srcPathlist[1], ":")
		host, srcPath = hostpath[0], &(hostpath[1])
		opstype = 0

	} else if strings.Contains(*dstPath, "@") {
		dstPathlist := strings.Split(*dstPath, "@")
		user = dstPathlist[0]
		hostpath := strings.Split(dstPathlist[1], ":")
		host, dstPath = hostpath[0], &(hostpath[1])
		opstype = 1
	} else {
		os.Exit(127)
	}
	cliConf := new(ClientConfig)
	cliConf.createClient(host, *port, user, *password)
	if opstype == 1 {
		//本地文件上传到服务器
		cliConf.Upload(*srcPath, *dstPath) // /root/haha.go
	} else {
		//从服务器中下载文件
		cliConf.Download(*srcPath, *dstPath) //文件下载完毕
	}
